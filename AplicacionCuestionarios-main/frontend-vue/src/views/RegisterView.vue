...
async function handleRegistro() {
  ...
  cargando.value = true
  const err = await registro(nombre.value.trim(), correo.value.trim(), password.value)
...
<template>
  <div class="auth-page">
    <div class="auth-card">
      ...
      <p class="auth-subtitle">Regístrate para crear y compartir cuestionarios</p>

      <form class="auth-form" @submit.prevent="handleRegistro" novalidate>
        ...
        <button id="btn-registro" type="submit" class="btn btn--primary"
      ...
      <p class="auth-footer">
        ¿Ya tienes cuenta?
        <router-link to="/" class="auth-link">Inicia sesión</router-link>
      </p>
    </div>
  </div>
</template>
...
.auth-title { font-size: var(--font-size-2xl); font-weight: 700; color: var(--color-text); margin: 0; text-align: center; }
.auth-subtitle { font-size: var(--font-size-sm); color: var(--color-text-muted); margin: 0; text-align: center; }
.auth-form { width: 100%; display: flex; flex-direction: column; gap: var(--space-4); }
.auth-footer { font-size: var(--font-size-sm); color: var(--color-text-muted); text-align: center; margin: 0; }
